...
Frontend source lives in `frontend/` and follows Next.js App Router.
...
- `zustand` store (`frontend/lib/studio-store.ts`) for global session/workspace/thread state.
- API client (`frontend/lib/api.ts`) maps directly to backend endpoints.
