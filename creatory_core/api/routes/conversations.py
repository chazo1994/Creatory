...
from creatory_core.api.permissions import (
    ensure_conversation_member,
    ensure_thread_in_conversation,
    ensure_workspace_member,
)
from creatory_core.db.models import Conversation, Message, Thread, ThreadKind, User
from creatory_core.db.session import get_db_session
from creatory_core.schemas.conversation import (
...
from creatory_core.services.bridge import inject_context_block
...
async def inject_context(
    ...
    bridge_result = await inject_context_block(
        db,
        conversation_id=conversation_id,
        ...
        payload_context_block=payload.context_block,
    )
    await db.commit()
    await db.refresh(bridge_result.context_injection)
    return ContextInjectionRead.model_validate(bridge_result.context_injection)
