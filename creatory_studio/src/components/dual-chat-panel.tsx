...
export function DualChatPanel({ token, conversationId, mainThreadId, quickThreadId }: Props) {
  ...
  const runChatMutation = useMutation({
    ...
    onSuccess: (response, variables) => {
      ...
      setLatestRunId(response.agent_run.id);
    }
  });

  const injectMutation = useContextInjection({
    token,
    conversationId,
  ...
  const { events: streamEvents, isStreaming } = useRunStream({
    ...
    <section className="panel p-3 lg:col-span-8">
        ...
        <div>
          ...
          {latestRunId ? (
            <p className="mt-1 text-xs text-black/55">
              Orchestration run: {latestRunId.slice(0, 8)} {isStreaming ? "streaming..." : ""}
            </p>
          ) : null}
        </div>
        <div className="text-right">
          ...
          {streamEvents.length > 0 ? (
            <p className="text-xs text-black/55">
              Latest event: {streamEvents[streamEvents.length - 1]?.event}
            </p>
          ) : null}
        </div>
      </header>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <ThreadColumn
          ...
          onInject={(message) => {
            injectMutation.mutate(message, {
              onSuccess: () => setInjectStatus("Injected into main context block."),
              onError: (error: Error) => setInjectStatus(error.message)
            });
          }}
        />
      </div>
